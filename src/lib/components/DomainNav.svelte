...
<div class="mt-8 mb-16">
  <a href="/map" class="text-indigo-500 dark:text-indigo-300 underline hover:text-indigo-700 transition block mb-8 text-sm">
    ← {t.backToMap}
  </a>
  ...
  <nav class="w-full max-w-6xl mx-auto flex flex justify-between gap-12 items-center text-sm text-indigo-400 px-4 sm:px-12 md:px-24 lg:px-40">
    <a href={`/map/${prev}`} class="hover:underline">← {t[prev]}</a>
    <a href={`/map/${next}`} class="hover:underline">{t[next]} →</a>
  </nav>
</div>
